...
void DefMesh::updateVertices()
{
    for(int i=1; i<=pmodel->numvertices; ++i)
    {
        ...
        for(int j=1; j<mySkeleton.joints.size(); ++j)
        {
			...
			float fweight = weights[(i - 1) * 17 + j - 1];

			if (fweight != 0) {
				float t[16];
				scalar(fweight, mySkeleton.joints[j].global_t, t);
				add(m, t, m);
			}
        }
        ...
        pmodel->vertices[i*3]  = v[0];
		pmodel->vertices[i*3+1]= v[1];
		pmodel->vertices[i*3+2]= v[2];
